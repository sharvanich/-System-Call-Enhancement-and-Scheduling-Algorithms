...
000000000000008e <runcmd>:
{
  ...
  exit(0);
      ea:	4501                	li	a0,0
      ec:	319000ef          	jal	c04 <exit>
      exit(1);
      f0:	4505                	li	a0,1
      f2:	313000ef          	jal	c04 <exit>
    close(rcmd->fd);
      f6:	5148                	lw	a0,36(a0)
      f8:	335000ef          	jal	c2c <close>
    if(open(rcmd->file, rcmd->mode) < 0){
      fc:	508c                	lw	a1,32(s1)
      fe:	6888                	ld	a0,16(s1)
     100:	345000ef          	jal	c44 <open>
    ...
    if(fork1() == 0){
     ...
     14c:	e90d                	bnez	a0,17e <runcmd+0xf0>
      close(1);
     14e:	4505                	li	a0,1
     150:	2dd000ef          	jal	c2c <close>
      dup(p[1]);
     154:	fdc42503          	lw	a0,-36(s0)
     158:	325000ef          	jal	c7c <dup>
      close(p[0]);
     15c:	fd842503          	lw	a0,-40(s0)
     160:	2cd000ef          	jal	c2c <close>
      close(p[1]);
     164:	fdc42503          	lw	a0,-36(s0)
     168:	2c5000ef          	jal	c2c <close>
      runcmd(pcmd->left);
     16c:	6488                	ld	a0,8(s1)
    ...
    if(fork1() == 0){
     ...
     182:	e115                	bnez	a0,1a6 <runcmd+0x118>
      close(0);
     184:	2a9000ef          	jal	c2c <close>
      dup(p[0]);
     188:	fd842503          	lw	a0,-40(s0)
     18c:	2f1000ef          	jal	c7c <dup>
      close(p[0]);
     190:	fd842503          	lw	a0,-40(s0)
     194:	299000ef          	jal	c2c <close>
      close(p[1]);
     198:	fdc42503          	lw	a0,-36(s0)
     19c:	291000ef          	jal	c2c <close>
      runcmd(pcmd->right);
     1a0:	6888                	ld	a0,16(s1)
     1a2:	eedff0ef          	jal	8e <runcmd>
    close(p[0]);
     1a6:	fd842503          	lw	a0,-40(s0)
     1aa:	283000ef          	jal	c2c <close>
    close(p[1]);
     1ae:	fdc42503          	lw	a0,-36(s0)
     1b2:	27b000ef          	jal	c2c <close>
    wait(0);
     1b6:	4501                	li	a0,0
     1b8:	255000ef          	jal	c0c <wait>
    wait(0);
     1bc:	4501                	li	a0,0
     1be:	24f000ef          	jal	c0c <wait>
    break;
...
stat(const char *n, struct stat *st)
{
  ...
  r = fstat(fd, st);
     ae6:	85ca                	mv	a1,s2
     ae8:	174000ef          	jal	c5c <fstat>
     aec:	892a                	mv	s2,a0
  close(fd);
     aee:	8526                	mv	a0,s1
     af0:	13c000ef          	jal	c2c <close>
  return r;
     af4:	64a2                	ld	s1,8(sp)
}
     af6:	854a                	mv	a0,s2
